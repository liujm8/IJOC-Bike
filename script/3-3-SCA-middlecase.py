...
G=nx.Graph()
veh_num=c-5
file_path='../data/3-2-MiddleCase/'
file_source='Case_30C.csv'
file_name=file_source.split('.')[0]
writer=csv.writer(open('../results/MiddleCase/'+file_name+'_SCA_'+str(veh_num)+'.csv','w',newline=''))
...
penalty=0
tt=0
station=np.loadtxt(file_path+file_source,delimiter=',',skiprows=1)
for row in station:
    if row[c]==0:
        penalty+=abs(row[3])
    else:
...
for row in classes:
    if row !=0:
        ...
        for v in range(veh_n):
            for i in range(n):
                for j in range(n):
                    if i != j and solution[v, i, j] > 0.5:
                        plt.plot([points[i][1], points[j][1]], [points[i][0], points[j][0]], 'o-',color=color_list[color_index],
                                 linewidth=2)
        color_index+=1
        for v in range(veh_n):
            for j in range(n):
                if solution[v, -1, j] > 0.5:
                    plt.plot([depot[1], points[j][1]], [depot[0], points[j][0]], color='r', linewidth=4)
        for v in range(veh_n):
            for j in range(n):
                if solution[v, j, -2] > 0.5:
                    plt.plot([depot[1], points[j][1]], [depot[0], points[j][0]], color='yellow', linewidth=4)

        plt.scatter(depot[1], depot[0], marker='+')
        # plt.show()
        for i in range(n):
            (xx, yy) = points[i]
            # print('dashlen call %d' % dl)
            t = ax.text(yy, xx, str(i))
...
plt.savefig('../results/MiddleCase/'+file_name+'_SCA_'+str(veh_num)+'.png',dpi=900)
print('total obj:',obj)
